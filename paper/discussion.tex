...
datasets, especially when a concept drift is involved \TG{well the Hoeffding tree is still very much superior when there is a concept drift.}. In which case the
...
is the only classifier that presents a variance\TG{so what? I would remove this comment}.
...
in the classification performance \TG{it sounds a bit obvious for a discussion, you could move this to results}.
...
sensor \TG{I dont find this justification very convincing}. It is cheaper to buy or build one sensor, and it is faster to setup.
\TG{what do you expect if multiple sensors are used?}
...
Note that we selected the memory bound does not match all situations\TG{this sentence doesn't parse}. Indeed,
depending on the connected object the memory limit can range from few KB to GB.
In this study, the \mondrianforest and the \FNN are both limited to 600~KB\TG{It's the first time that a memory limitation for FNN is mentioned}.

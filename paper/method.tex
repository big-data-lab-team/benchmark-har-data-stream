...
Neighbors~\cite{mc-nn} is a compressed version of the k-nearest neighbor~(\knn) \TG{introduce the acronym the first time you use it}
...
merged \TG{past tense, next paragraphs are present tense. To be harmonized.} some of them together based on the table
in~\cite{behzad2019}. 
...
and gyroscope) \TG{and one sensor only?}
...
fit for numerical attributes. \TG{You should say that this was also reimplemented in OrpailleCC and explain why: 
Two implementations were used, the OrpailleCC one and the StreamDM one. We used two implementations 
...
\TG{here the text shifts from past tense to present tense, this should be harmonized}

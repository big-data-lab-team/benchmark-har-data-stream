		...
		if(E >= 0 && parent_tau + E < node.tau){//Introduce a new parent and a new sibling
			...
			if(new_parent >= 0){
				...
				if(new_sibling >= 0){
					...
					for(int i = 0; i < feature_count; ++i){
						nodes[new_parent].set_bound_lower(i, features[i] < node.bound_lower[i] ? features[i] : node.bound_lower[i]);
						nodes[new_parent].set_bound_upper(i, features[i] > node.bound_upper[i] ? features[i] : node.bound_upper[i]);
					}
		...
		if(update_box){ //Otherwise, just update the box
			...
			for(int i = 0; i < feature_count; ++i){
				if(node.bound_lower[i] > features[i])
					node.set_bound_lower(i, features[i]); 
				if(node.bound_upper[i] < features[i])
					node.set_bound_upper(i, features[i]); 
			}
